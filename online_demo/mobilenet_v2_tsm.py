...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        ...
        self.last_channel = make_divisible(last_channel * width_mult) if width_mult > 1.0 else last_channel      # 딱히 신경x
        self.features = [conv_bn(3, input_channel, 2)]                                                           # conv2d + BN + ReLU conv_bn(in = 3, out=32, stride=2) 
        # building inverted residual blocks
        global_idx = 0
        shift_block_idx = [2, 4, 5, 7, 8, 9, 11, 12, 14, 15]                #  일부 블럭들을 옮기려고 하는 블럭들의 번호?
        for t, c, n, s in interverted_residual_setting:
            output_channel = make_divisible(c * width_mult) if t > 1 else c     # c값이 아마 그대로 출력 16 > 24 > 32 > 64 > 96 > 160 > 320
            for i in range(n):
                if i == 0:
